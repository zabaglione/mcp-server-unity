...
export class PerformanceBenchmark {
     ...
     */
    async runBenchmark(name, operation, iterations = 100, warmup = 10) {
        console.log(`\n📊 Benchmarking: ${name}`);
        ...
        for (const suite of suites) {
            ...
            for (const benchmark of suite.benchmarks) {
                try {
                    ...
                    // Run benchmark
                    const result = await this.runBenchmark(benchmark.name, benchmark.operation, benchmark.iterations, benchmark.warmup);
