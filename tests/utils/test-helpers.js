...
export class TestHelpers {
     ...
     */
    static setupMockFileSystem(structure) {
        const mockReadFile = jest.fn(async (filePath) => {
        ...
        const mockWriteFile = jest.fn(async (filePath, content) => {
            const normalizedPath = path.normalize(filePath);
            structure[normalizedPath] = content;
        });
        const mockMkdir = jest.fn(async (dirPath) => {
        ...
        const mockReaddir = jest.fn(async (dirPath) => {
            const normalizedPath = path.normalize(dirPath);
     ...
     */
    static createMockUnityProject(projectPath) {
        return {
     ...
     */
    static createMockUnityProjectWithURP(projectPath) {
     ...
     */
    static createMockUnityProjectWithHDRP(projectPath) {
        const base = TestHelpers.createMockUnityProject(projectPath);
        base[path.join(projectPath, 'Packages', 'manifest.json')] = JSON.stringify({
            dependencies: {
